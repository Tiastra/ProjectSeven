...
$titulo = $tituloController->buscar($_GET['id']);

$clienteController = new ClienteController();
$clienteSelecionado = $clienteController->buscar($titulo->id_cliente);
...
<div class="w3-container" id="contact">
    ...
    <form action="editar.php?id=<?=$titulo->id_titulo?>" method="post">
        <h4>Dados do Título</h4>
        <p>
            <label>
                <select class="w3-select w3-padding-16 w3-border w3-margin-bottom" name="id_cliente" required>
                    <option class="w3-select w3-large" value="<?=$clienteSelecionado->id_cliente?>" selected><?=$clienteSelecionado->nome_cliente?></option>
            </label>
            <?=$resultados?>
        </p>
        <p>
            <label>
                <input class="w3-input w3-padding-16 w3-border w3-margin-top" type="text" placeholder="Informações da Dívida" required name="descricao_titulo" value="<?=$titulo->descricao_titulo?>">
            </label>
        </p>
        <p>
            <label>
                <input class="w3-input w3-padding-16 w3-border" type="number" placeholder="Valor R$" required name="valor_titulo" value="<?=$titulo->valor_titulo?>">
            </label>
        </p>
        <p>
            <label>Data de Vencimento
                <input class="w3-input w3-padding-16 w3-border" type="date" placeholder="Data de Nascimento" required name="data_vencimento" value="<?=$titulo->data_vencimento?>">
            </label>
        </p>
